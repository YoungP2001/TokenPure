...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(
        ...
        self.vae_scale_factor = (
            2 ** (len(self.vae.config.block_out_channels)) if hasattr(self, "vae") and self.vae is not None else 8
        )
    ...
    def prepare_latents(
        ...
        noise_latents = self._pack_latents(latents, batch_size, num_channels_latents, height, width)
        ...
        if subject_image is not None:
            ...
            subject_latents = self._pack_latents(subject_image_latents, batch_size, num_channels_latents,
                                                 height_cond * sub_number, width_cond)
        ...
        if condition_image is not None:
            ...
            cond_latents = self._pack_latents(image_latents, batch_size, num_channels_latents,
                                              height_cond * cond_number, width_cond)
    ...
    # def set_ip_adapter(self, ip_adapter):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    ...
                    cond_hidden_states=cond_latents,  # 1x1024x64
                    timestep=timestep / 1000,
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds,
                    encoder_hidden_states=prompt_embeds,
                    image_emb=image_emb,  # 1x128x4096
                    txt_ids=text_ids,
